---
title: "Variable Selection Swiss Day Ahead prices"
...
```{r}
# Read data
data <- read_csv('data_cleaned_UTC_new.csv')
df <- data
```
```{r}
...
for (i in 1:length(lasso_grid)){
  ...
  train_metrics = append(
    ...
    lasso_fit |> 
      predict(dt_train) |> 
      mutate(actual = dt_train$day_ahead_price_ch) |> 
      rmse(actual, .pred) |> 
      pull(.estimate)
  )
  ...
  test_metrics = append(
    ...
    lasso_fit |> 
      predict(dt_test) |> 
      mutate(actual = dt_test$day_ahead_price_ch) |> 
      rmse(actual, .pred) |> 
      pull(.estimate)
  )
...
```{r}
# Create recipe
rf_rec = recipe(day_ahead_price_ch ~ ., data = dt_train) |>
  step_mutate(hour = hour(date)) |> 
  step_date(date) |> 
  update_role(date, new_role = "date") |> 
  step_zv(all_predictors()) |> 
...
data <- read.csv('data_cleaned_UTC_new.csv')  
split_date = ymd_hm("2023-01-01 00:00")
data <- data %>% 
  filter(date >= split_date )
```
