...
and pinpointed highly correlated variables. This insight may prove invaluable in a subsequent phase for addressing multicollinearity during modeling.
...
I shift the covariates of one day, this means that the date for the data frame df makes sense only for day_ahead_price_de. df will be used in reduction with model.
...
```{r}
# Fitting the model
lasso_fit = fit_lasso(alpha = 0.1500025) #min test
...
write.csv(df, "data_shifted_one_day.csv", row.names = FALSE)
```