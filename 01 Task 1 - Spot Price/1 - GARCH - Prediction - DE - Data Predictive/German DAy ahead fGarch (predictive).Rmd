---
title: "German DA prices fGarch(theoretical)"
...
```{r}
df_CH_DE <- read.csv("~/Downloads/0_df_predictive_de_spot_price.csv", header=TRUE)



df_CH_DE$cal_day_in_month = as.factor(df_CH_DE$cal_day_in_month)
df_CH_DE$cal_day_in_week = as.factor(df_CH_DE$cal_day_in_week)
df_CH_DE$cal_hour_in_day= as.factor(df_CH_DE$cal_hour_in_day)
df_CH_DE$cal_month = as.factor(df_CH_DE$cal_month)
df_CH_DE$cal_quarter = as.factor(df_CH_DE$cal_quarter)
df_CH_DE$cal_year = as.factor(df_CH_DE$cal_year)



df_DE_DA_train= arrange(df_CH_DE, date)[1:8734,]
df_DE_DA_test= arrange(df_CH_DE, date)[8735:9477,]
```
...
# Prepare your data
response <- df_DE_DA_train$day_ahead_price_de
external_regressors <- df_DE_DA_train[, c("day_ahead_price_de", "day_ahead_price_at")]  # Replace with your actual regressor names
...
response_test= df_DE_DA_test$day_ahead_price_de[0:prediction_horizon]
...
df_combined <- df_DE_DA_train  # Assuming this data frame includes all necessary variables
...
for (k in seq(1:nrow(df_DE_DA_test))) {
    if (i + prediction_horizon > nrow(df_DE_DA_test)) {
  ...
  print(i)
    ...
    # Fit GARCH model
    response <- df_combined$day_ahead_price_de
  ...
  print("check 2")
    ...
    new_data <- df_DE_DA_test[i:(i + prediction_horizon - 1), ]
...
```
```{r}
save(all_forecasts, file = "forecasts_theoretical_German_DA_fGarch.RData")
print("saved")
```
...
df_scores= data.frame(date= df_DE_DA_test$date[1:720], .pred= forecast_values, acutal=response_test )
