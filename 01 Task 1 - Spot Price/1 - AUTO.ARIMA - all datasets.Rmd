...
table_fit<- function(fit, target, setting="predictive"){
  coefficients <- coef(fit)
  se <- sqrt(diag(vcov(fit)))  # Standard errors
  ci <- confint(fit, level = 0.95)           # Confidence intervals
  results <- data.frame(
  ...
  setting_clean <- gsub("_", " ", setting)

  knitr::kable(results, align = "c", caption = paste0("ARIMA Model for ", target_clean, " - ", setting_clean, " setting Coefficients and Confidence Intervals"), format = "latex") %>% cat(file = "tables_ARMA.tex", append = TRUE)
}
