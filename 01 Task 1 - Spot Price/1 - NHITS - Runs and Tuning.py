...
# Create splits
df_train = df.query("date < @train_end").reset_index(drop=True)
df_val = df.query("date < @val_end").query("date >= @train_end").reset_index(drop=True)
df_test = df.query("date >= @val_end").reset_index(drop=True)
...
df_val = pd.concat(
    [
        df_val["date"].reset_index(drop=True),
        y_val.reset_index(drop=True),
        X_val_preprocessed.reset_index(drop=True),
    ],
    axis=1,
)
...
df_val = df_val.rename(columns={"day_ahead_price_ch": "y", "date": "ds"})
...
def objective(trial):
    ...
    nf.fit(df=df_train, val_size=int(0.2*df_train.shape[0]))

    # Make predictions on the validation set and get validation loss
    val_pred_dfs = []
    pred_timestamps = df_val.query("ds.dt.hour == 10")["ds"].reset_index(drop=True)
    ts = pred_timestamps[10]
    
    for ts in pred_timestamps:
        start_ts = ts - pd.Timedelta(lookback_window, unit="hours")
        end_ts = ts + pd.Timedelta(HORIZON_SIZE, unit="hours")
        pred_day = ts.date() + pd.Timedelta(1, unit="days")
        df_pred = df_val.query("@start_ts <= ds <= @end_ts").reset_index(drop=True)
