...
```{r}
best_models <- model_selected(models, response = 'German price' )
```

```{r}
results <- prediction_step(models, auction = F, inverse_transformation = inverse_ihs10, response = 'German price')
```
```{r}
results$result %>% 
  mutate(.pred = fc_srb) %>% 
