...
garch_model_DA_CH <- ugarchspec(mean.model = list(armaOrder = c(3,3), include.mean = TRUE, external.regressors = as.matrix(select(train_data_CH_DA,-c("asinh_price_ch")))), variance.model = list(model = "sGARCH",garchOrder = c(3, 3), external.regressors = as.matrix(select(train_data_CH_DA,-c("asinh_price_ch")))), distribution.model = "norm")

  
fit_DA_CH_ARMA_GARCH <- ugarchfit(spec = garch_model_DA_CH, data = train_data_CH_DA, solver="hybrid")

start_pars <- coef(fit_DA_CH_ARMA_GARCH)
...
for (i in seq(1, nrow(df_CH_DA_test), by = chunk_size)) {
  ...
  fit_DA_CH_ARMA_GARCH <- ugarchfit(spec = garch_model_DA_CH, data = train_data_CH_DA, solver="hybrid", start.pars = start_pars)
  print("check 1: " )
...
# Print the result
print(c("r2", R2_DA_CH, "mse:",mse_DA_CH ))

```

```{r}
save(predictions_DA_CH,fit_DA_CH_ARMA_GARCH,file = "Swiss DA price w. retrining (ugarch) 00:41: FINAL PICTURES.RData") 
print("saved")
```
```{r}
#Visualization of out of sample fit:
