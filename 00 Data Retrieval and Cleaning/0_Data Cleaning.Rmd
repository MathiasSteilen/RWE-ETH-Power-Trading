...
df_scenario_c = df |> 
  mutate(
    across(
      contains("actual_load"), 
      ~ lag(., n = 24), 
      .names = "{.col}_lag_24"
    ),
    across(
      contains("biomass_actual_aggregated"), 
      ~ lag(., n = 24), 
      .names = "{.col}_lag_24"
    ),
    across(
      contains("capacity_forecast"), 
      ~ lag(., n = 24), 
      .names = "{.col}_lag_24"
    ),
    across(
      contains("crossborder_actual_flow"), 
      ~ lag(., n = 24), 
      .names = "{.col}_lag_24"
    ),
    across(
      contains("fossil_brown_coal_lignite_actual_aggregated"), 
      ~ lag(., n = 24), 
      .names = "{.col}_lag_24"
    ),
    across(
      contains("fossil_coal.derived_gas_actual_aggregated"), 
      ~ lag(., n = 24), 
      .names = "{.col}_lag_24"
    ),
    across(
      contains("fossil_gas_actual_aggregated"), 
      ~ lag(., n = 24), 
      .names = "{.col}_lag_24"
    ),
    across(
      contains("fossil_hard_coal_actual_aggregated"), 
      ~ lag(., n = 24), 
      .names = "{.col}_lag_24"
    ),
    across(
      contains("fossil_oil_actual_aggregated"), 
      ~ lag(., n = 24), 
      .names = "{.col}_lag_24"
    ),
    across(
      contains("geothermal_actual_aggregated"), 
      ~ lag(., n = 24), 
      .names = "{.col}_lag_24"
    ),
    across(
      contains("hydro_pumped_storage"), 
      ~ lag(., n = 24), 
      .names = "{.col}_lag_24"
    ),
    across(
      contains("hydro_pumped_storage_actual_aggregated"), 
      ~ lag(., n = 24), 
      .names = "{.col}_lag_24"
    ),
    across(
      contains("hydro_pumped_storage_actual_consumption"), 
      ~ lag(., n = 24), 
      .names = "{.col}_lag_24"
    ),
    across(
      contains("hydro_reservoir_storage"), 
      ~ lag(., n = 24), 
      .names = "{.col}_lag_24"
    ),
    across(
      contains("hydro_run.of.river_and_poundage"), 
      ~ lag(., n = 24), 
      .names = "{.col}_lag_24"
    ),
    across(
      contains("hydro_run.of.river_and_poundage_actual_aggregated"), 
      ~ lag(., n = 24), 
      .names = "{.col}_lag_24"
    ),
    across(
      contains("hydro_water_reservoir"), 
      ~ lag(., n = 24), 
      .names = "{.col}_lag_24"
    ),
    across(
      contains("hydro_water_reservoir_actual_aggregated"), 
      ~ lag(., n = 24), 
      .names = "{.col}_lag_24"
    ),
    across(
    ...
    across(
      contains("nuclear_actual_aggregated"), 
      ~ lag(., n = 24), 
      .names = "{.col}_lag_24"
    ),
    across(
