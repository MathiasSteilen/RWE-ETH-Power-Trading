...
df <- read_csv("../00 Data Retrieval and Cleaning/0_df_predictive_chde_auction_price.csv") %>% 
...
redundant_columns <- lm(auction_price_ch_de ~ ., data = df) %>%  
  broom::tidy() |> 
...
ff <- formula(auction_price_ch_de ~ -1 + .^2) #only numeric
X1 <- df %>% 
...
X2 <- df %>%
  select(-c(auction_price_ch_de, date)) %>% 
  select(-contains(dummies_columns)) %>% 
...
```{r}
models <- df %>% 
  mutate(auction_price_ch_de = ihs10(auction_price_ch_de)) %>% 
  model_list(p.train = p.train, n_lags_max = max_lag, response = 'auction_price_ch_de')
```

```{r}
best_models <- model_selected(models, response = 'Transmission price CH-DE')
```

```{r}
results <- prediction_step(models, auction = T, inverse_transformation = inverse_ihs10, response = 'Transmission price CH-DE')
```
...
results$result %>% 
  ...
  write.csv(file = 'holdout_predictions.csv')
```
...
df <- read_csv("../00 Data Retrieval and Cleaning/0_df_theoretical_chde_auction_price.csv") %>% 
  filter(date >= time_cut)
