...
This file is specific to the day_ahead_price_ch.
...
```{r}
mod_a_1_ch <- df %>% 
  model_list(nlag = lag_1, response = 'day_ahead_price_ch')
saveRDS(mod_a_1_ch, "mod_a_1_ch.rds")
rm(mod_a_1_ch)
```
...
mod_a_2_ch <- df  %>% 
  ...
  filter(date < first_cut) %>% 
  model_list(nlag = lag_2, response = 'day_ahead_price_ch') 
saveRDS(mod_a_2_ch, "mod_a_2_ch.rds")
rm(mod_a_2_ch)
```
...
mod_a_3_ch <- df  %>% 
  select(-any_of(columns_to_remove)) %>%
  filter(date > first_cut & date < second_cut) %>% 
  model_list(nlag = lag_3, response = 'day_ahead_price_ch') 
saveRDS(mod_a_3_ch, "mod_a_3_ch.rds")
rm(mod_a_3_ch)
```
...
saveRDS(mod_a_4_ch, "mod_a_4_ch.rds")
rm(mod_a_4_ch)
```
...
mod_b_1_ch <- df %>% 
  model_list(nlag = lag_1, response = 'day_ahead_price_ch')
saveRDS(mod_b_1_ch, "mod_b_1_ch.rds")
rm(mod_b_1_ch)
```
...
mod_b_2_ch <- df  %>% 
  ...
  filter(date < first_cut) %>% 
  model_list(nlag = lag_2, response = 'day_ahead_price_ch') 
saveRDS(mod_b_2_ch, "mod_b_2_ch.rds")
rm(mod_b_2_ch)
```
...
mod_b_3_ch <- df  %>% 
  ...
  filter(date > first_cut & date < second_cut) %>% 
  model_list(nlag = lag_3, response = 'day_ahead_price_ch') 
saveRDS(mod_b_3_ch, "mod_b_3_ch.rds")
rm(mod_b_3_ch)
```
...
mod_b_4_ch <- df  %>% 
  ...
  filter(date > second_cut) %>% 
  model_list(nlag = lag_4, response = 'day_ahead_price_ch') 
saveRDS(mod_b_4_ch, "mod_b_4_ch.rds")
rm(mod_b_4_ch)
```
...
mod_c_1_ch <- df %>% 
  model_list(nlag = lag_1, response = 'day_ahead_price_ch')
saveRDS(mod_c_1_ch, "mod_c_1_ch.rds")
rm(mod_c_1_ch)
```
...
mod_c_2_ch <- df  %>% 
  ...
  filter(date < first_cut) %>% 
  model_list(nlag = lag_2, response = 'day_ahead_price_ch') 
saveRDS(mod_c_2_ch, "mod_c_2_ch.rds")
rm(mod_c_2_ch)
```
...
mod_c_3_ch <- df  %>% 
  ...
  filter(date > first_cut & date < second_cut) %>% 
  model_list(nlag = lag_3, response = 'day_ahead_price_ch') 
saveRDS(mod_c_3_ch, "mod_c_3_ch.rds")
rm(mod_c_3_ch)
```
...
mod_c_4_ch <- df  %>% 
  ...
  filter(date > second_cut) %>% 
  model_list(nlag = lag_4, response = 'day_ahead_price_ch') 
saveRDS(mod_c_4_ch, "mod_c_4_ch.rds")
rm(mod_c_4_ch)
```