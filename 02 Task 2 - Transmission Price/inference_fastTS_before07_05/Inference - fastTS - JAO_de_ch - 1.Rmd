...
```{r}
mod_a_1_de_ch <- df %>% 
  model_list(nlag = lag_1, response = 'auction_price_de_ch')
saveRDS(mod_a_1_de_ch, "mod_a_1_de_ch.rds")
rm(mod_a_1_de_ch)
```


When we subset some of the dummies colums of the date_variable are all 1, so we remove them from the covariate matrix. This is done for all the following chunk. 
...
mod_a_2_de_ch <- df  %>%
  ...
  filter(date < first_cut) %>% 
  model_list(nlag = lag_2, response = 'auction_price_de_ch')

saveRDS(mod_a_2_de_ch, "mod_a_2_de_ch.rds")
rm(mod_a_2_de_ch)
```
...
mod_a_3_de_ch <- df  %>% 
  select(-any_of(columns_to_remove)) %>%
  filter(date > first_cut & date < second_cut) %>% 
  model_list(nlag = lag_3, response = 'auction_price_de_ch') 
saveRDS(mod_a_3_de_ch, "mod_a_3_de_ch.rds")
rm(mod_a_3_de_ch)
```
...
mod_a_4_de_ch <- df  %>% 
  select(-any_of(columns_to_remove)) %>%
  filter(date > second_cut) %>% 
  model_list(nlag = lag_4, response = 'auction_price_de_ch') 
saveRDS(mod_a_4_de_ch, "mod_a_4_de_ch.rds")
rm(mod_a_4_de_ch)
```
...
mod_b_1_de_ch <- df %>% 
  model_list(nlag = lag_1, response = 'auction_price_de_ch')
saveRDS(mod_b_1_de_ch, "mod_b_1_de_ch.rds")
rm(mod_b_1_de_ch)
```
...
mod_b_2_de_ch <- df  %>% 
  ...
  filter(date < first_cut) %>% 
  model_list(nlag = lag_2, response = 'auction_price_de_ch') 
saveRDS(mod_b_2_de_ch, "mod_b_2_de_ch.rds")
rm(mod_b_2_de_ch)
```
...
mod_b_3_de_ch <- df  %>% 
  ...
  filter(date > first_cut & date < second_cut) %>% 
  model_list(nlag = lag_3, response = 'auction_price_de_ch') 
saveRDS(mod_b_3_de_ch, "mod_b_3_de_ch.rds")
rm(mod_b_3_de_ch)
```
...
mod_b_4_de_ch <- df  %>% 
  ...
  filter(date > second_cut) %>% 
  model_list(nlag = lag_4, response = 'auction_price_de_ch') 
saveRDS(mod_b_4_de_ch, "mod_b_4_de_ch.rds")
rm(mod_b_4_de_ch)
```
...
mod_c_1_de_ch <- df %>% 
  model_list(nlag = lag_1, response = 'auction_price_de_ch')
saveRDS(mod_c_1_de_ch, "mod_c_1_de_ch.rds")
rm(mod_c_1_de_ch)
```
...
mod_c_2_de_ch <- df  %>% 
  ...
  filter(date < first_cut) %>% 
  model_list(nlag = lag_2, response = 'auction_price_de_ch') 
saveRDS(mod_c_2_de_ch, "mod_c_2_de_ch.rds")
rm(mod_c_2_de_ch)
```
...
mod_c_3_de_ch <- df  %>% 
  ...
  filter(date > first_cut & date < second_cut) %>% 
  model_list(nlag = lag_3, response = 'auction_price_de_ch') 
saveRDS(mod_c_3_de_ch, "mod_c_3_de_ch.rds")
rm(mod_c_3_de_ch)
```
...
mod_c_4_de_ch <- df  %>% 
  ...
  filter(date > second_cut) %>% 
  model_list(nlag = lag_4, response = 'auction_price_de_ch') 
saveRDS(mod_c_4_de_ch, "mod_c_4_de_ch.rds")
rm(mod_c_4_de_ch)
```