...
This file is specific to the day_ahead_price_de.
...
```{r}
mod_a_1_de <- df %>% 
  model_list(nlag = lag_1)
saveRDS(mod_a_1_de, "mod_a_1_de.rds")
rm(mod_a_1_de)
```
...
mod_a_2_de <- df  %>% 
  ...
  filter(date < first_cut) %>% 
  model_list(nlag = lag_2) 
saveRDS(mod_a_2_de, "mod_a_2_de.rds")
rm(mod_a_2_de)
```
...
mod_a_3_de <- df  %>% 
  select(-any_of(columns_to_remove)) %>%
  filter(date > first_cut & date < second_cut) %>% 
  model_list(nlag = lag_3) 
saveRDS(mod_a_3_de, "mod_a_3_de.rds")
rm(mod_a_3_de)
```
...
saveRDS(mod_a_4_de, "mod_a_4_de.rds")
rm(mod_a_4_de)
```
...
mod_b_1_de <- df %>% 
  model_list(nlag = lag_1)
saveRDS(mod_b_1_de, "mod_b_1_de.rds")
rm(mod_b_1_de)
```
...
mod_b_2_de <- df  %>% 
  ...
  filter(date < first_cut) %>% 
  model_list(nlag = lag_2) 
saveRDS(mod_b_2_de, "mod_b_2_de.rds")
rm(mod_b_2_de)
```
...
mod_b_3_de <- df  %>% 
  ...
  filter(date > first_cut & date < second_cut) %>% 
  model_list(nlag = lag_3) 
saveRDS(mod_b_3_de, "mod_b_3_de.rds")
rm(mod_b_3_de)
```
...
mod_b_4_de <- df  %>% 
  ...
  filter(date > second_cut) %>% 
  model_list(nlag = lag_4) 
saveRDS(mod_b_4_de, "mod_b_4_de.rds")
rm(mod_b_4_de)
```
...
mod_c_1_de <- df %>% 
  model_list(nlag = lag_1)
saveRDS(mod_c_1_de, "mod_c_1_de.rds")
rm(mod_c_1_de)
```
...
mod_c_2_de <- df  %>% 
  ...
  filter(date < first_cut) %>% 
  model_list(nlag = lag_2) 
saveRDS(mod_c_2_de, "mod_b_c_de.rds")
rm(mod_c_2_de)
```
...
mod_c_3_de <- df  %>% 
  ...
  filter(date > first_cut & date < second_cut) %>% 
  model_list(nlag = lag_3) 
saveRDS(mod_c_3_de, "mod_c_3_de.rds")
rm(mod_c_3_de)
```
...
mod_c_4_de <- df  %>% 
  ...
  filter(date > second_cut) %>% 
  model_list(nlag = lag_4) 
saveRDS(mod_c_4_de, "mod_c_4_de.rds")
rm(mod_c_4_de)
```