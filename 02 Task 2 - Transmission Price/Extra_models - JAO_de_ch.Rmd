---
title: "Extreme_models JAO_de_ch"
...
df <- df %>%
  filter(date > second_cut) %>% 
  mutate(dummy_ceiling = auction_price_de_ch <= boxcox_transform(0, lambda = -1.5)) %>% 
  mutate(dummy_ceiling = as.numeric(dummy_ceiling))
```
