...
```{r}
min_value <- df %>%
  summarize(minimum = min(auction_price_de_ch)) %>% 
  pull()
```
...
models1 <- df %>% 
  ...
  model_list(p.train = p.train, response = 'auction_price_de_ch', nlag = max_lag)
...
models3 <- df %>% 
  mutate(auction_price_de_ch = log(auction_price_de_ch  + 1)) %>% 
  model_list_prediction(ptrain = p.train, response = 'auction_price_de_ch', n_lags_max = max_lag)
