...
```{r}
min_value <- df %>%
  summarize(minimum = min(day_ahead_price_de)) %>% 
  pull()
```
...
models1 <- df %>% 
  mutate(day_ahead_price_de = log(day_ahead_price_de - min(day_ahead_price_de) + 1)) %>% 
  model_list(p.train = p.train, response = 'day_ahead_price_de', nlag = max_lag)
...
models3 <- df %>% 
  mutate(day_ahead_price_de = log(day_ahead_price_de - min(day_ahead_price_de) + 1)) %>% 
  model_list_prediction(ptrain = p.train, response = 'day_ahead_price_de', n_lags_max = max_lag)
