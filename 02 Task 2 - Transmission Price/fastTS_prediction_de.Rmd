...
models3 <- df %>% 
  mutate(day_ahead_price_de = log(day_ahead_price_de - min(day_ahead_price_de) + 1)) %>% 
  model_list_prediction(ptrain = 0.7, response = 'day_ahead_price_de', n_lags_max = max_lag)
...
# German Price
```{r}
df <- read_csv("../00 Data Retrieval and Cleaning/0_df_final_imputed_shifting_c_de.csv") %>% 
  filter(date >= time_cut)
