...
```{r}
df_auct <- read.csv("~/Downloads/0_df_predictive_chde_auction_price.csv", header=TRUE)
...
# Prepare your data
response <- df_auct_train$auction_price_ch_de
...
response_test= df_auct_test$auction_price_ch_de[0:prediction_horizon]
...
for (k in seq(1:nrow(df_auct_test))) {
  ...
  print(i)
    ...
    # Fit GARCH model
    response <- df_combined$auction_price_ch_de
